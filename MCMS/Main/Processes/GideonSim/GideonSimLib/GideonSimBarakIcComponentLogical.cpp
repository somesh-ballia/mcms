...
////////////////////////////////////////////////////////////////////////////
//
//   CBarakIcComponent - class treats all IC (Ivr Controller) commands (PlayMessage, etc.) in BARAK
//
////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////
...
//               Action table
/////////////////////////////////////////////////////////////////////////////
PBEGIN_MESSAGE_MAP(CBarakIcComponent)
	// Timer for sending IVR play message responses
	ONEVENT( IC_IVR_PLAY_RESPONSE_TOUT, ANYCASE, CBarakIcComponent::OnIvrRespToutAnycase)
PEND_MESSAGE_MAP(CBarakIcComponent,CStateMachine)


/////////////////////////////////////////////////////////////////////////////
CBarakIcComponent::CBarakIcComponent(CGideonSimBarakLogical* pBarakModule,
                           CTaskApp *pOwnerTask)
        :CStateMachine(pOwnerTask)
{
	m_pBarakModule = pBarakModule;
...
/////////////////////////////////////////////////////////////////////////////
CBarakIcComponent::~CBarakIcComponent()
{
...
/////////////////////////////////////////////////////////////////////////////
void* CBarakIcComponent::GetMessageMap()
{
...
/////////////////////////////////////////////////////////////////////////////
void CBarakIcComponent::HandleEvent(CSegment* pMsg,DWORD msgLen,OPCODE opCode)
{
...
/////////////////////////////////////////////////////////////////////////////
void CBarakIcComponent::OnMcmsReq( CMplMcmsProtocol* pMplProtocol )
{
...
//			destRollCallFile += 		"/testtest.wav";
			...
			for( int i=0; i<(int)pReqStruct->numOfMediaFiles; i++ )
			{
				...
				if (action == IVR_ACTION_TYPE_RECORD)
				{
					...
					PTRACE2(eLevelInfoNormal,"CBarakIcComponent::OnMcmsReq - IVR_PLAY_MESSAGE_REQ. Path: ",pszFileName);
...
/////////////////////////////////////////////////////////////////////////////
void CBarakIcComponent::OnIvrRespToutAnycase(CSegment* pParam)
{
	// endless loop of timer
	StartTimer(IC_IVR_PLAY_RESPONSE_TOUT,IC_IVR_RESPONSE_TIME);
	...
	for( int i=0; i<MAX_IC_IVR_PLAY_MSGS; i++ )
	{
		if( NULL != m_aIvrPlayMsg[i].pProtocol )
		{
			if( 0 >= m_aIvrPlayMsg[i].nTimeRemain )
			{
				...
				if( TRUE == m_pBarakModule->IsValidParty(*(m_aIvrPlayMsg[i].pProtocol)) ||
					   1 == m_pBarakModule->IsConfPlayMessage(*m_aIvrPlayMsg[i].pProtocol) )
				{
					switch( m_aIvrPlayMsg[i].pProtocol->getOpcode() )
					{
						case IVR_RECORD_ROLL_CALL_REQ:
						{
							...
							m_pBarakModule->SendToCmForMplApi(*(m_aIvrPlayMsg[i].pProtocol));	// send to MCMS
							break;
						}
						case IVR_PLAY_MESSAGE_REQ:
						default:
						{
							m_pBarakModule->Ack(*(m_aIvrPlayMsg[i].pProtocol),STATUS_OK);
							break;
						}
					}
				}
				else
					TRACESTR(eLevelInfoNormal) << " CBarakIcComponent::OnIvrRespToutAnycase - party not valid (disconnected?) ";
				// delete protocol
				POBJDELETE(m_aIvrPlayMsg[i].pProtocol);
			}
			else
...
/////////////////////////////////////////////////////////////////////////////
void CBarakIcComponent::AddIvrPlayMsgToQ(CMplMcmsProtocol& rMplProt)
{
	// check opcode
	...
	{
		m_pBarakModule->Ack(rMplProt,STATUS_OK);
	}
	else
	{
		// place msg to queue
		...
		if( MAX_IC_IVR_PLAY_MSGS == i)
			PTRACE(eLevelInfoNormal,"CBarakIcComponent::AddIvrPlayMsgToQ - no free place");
	}
...
/////////////////////////////////////////////////////////////////////////////
void CBarakIcComponent::AddIvrRollcallRecordToQ(CMplMcmsProtocol& rMplProt)
{
	// check opcode
	...
	if( MAX_IC_IVR_PLAY_MSGS == i)
		PTRACE(eLevelInfoNormal,"CBarakIcComponent::AddIvrRollcallRecordToQ - no free place");
}
