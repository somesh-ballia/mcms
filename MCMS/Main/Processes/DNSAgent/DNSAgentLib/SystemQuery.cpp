						...
						WORD& resPort, WORD& resPriority, WORD& resWeight)
{
    ...
    else
    {
    	...
    	if(eTransportTypeTcp == transportType)
    		snprintf(request, sizeof(request), "_sip._tcp.%s", domain);
    	else
    		if(eTransportTypeUdp == transportType)
    			snprintf(request, sizeof(request), "_sip._udp.%s", domain);
...
{
	int nRc = -1;
	if((NULL != par_HostName)&&(NULL != par_pOutBuff)&&(0 < par_nOutBuffLen))
	{
		char szCompleteHostName[PLCM_DNS_HOST_NAME_SIZE]="";
		
		if(  ((NULL != par_DomainName)&&('\0' != par_DomainName[0]))
           &&((PLCM_DNS_HOST_NAME_SIZE -2)> (strlen(par_HostName)+strlen(par_DomainName)) )
		  )
			snprintf(szCompleteHostName, sizeof(szCompleteHostName)-1, "%s.%s", par_HostName, par_DomainName);
		else
			strncpy(szCompleteHostName, par_HostName, sizeof(szCompleteHostName)-1);

		int nRequest_Query_Type	=	0;// 0= ns_o_query = 0,		/* Standard query. */
		nRc = res_mkquery(nRequest_Query_Type, szCompleteHostName
			    , C_IN
				, T_A 
				, NULL, 0, NULL, par_pOutBuff, par_nOutBuffLen);

		DNS_PACKET_HEADER		* pDnsHeader = (DNS_PACKET_HEADER *) par_pOutBuff;
		char					* pHostName  = (char*) par_pOutBuff + sizeof(DNS_PACKET_HEADER) + 1;
		int                       nHostNameLen = 0;
		{
			int nS = 0;
			for(nS = 0; nS < 256; nS++)
		...
		DNS_REQUEST_TYPE_CLASS	* pTypeClass = (DNS_REQUEST_TYPE_CLASS*) (&pHostName[nHostNameLen]);
		pTypeClass->wType = htons(par_eReqType);
		pDnsHeader->wId = htons(par_wReqId);
	}
	return nRc;
}
//======================================================================================//
