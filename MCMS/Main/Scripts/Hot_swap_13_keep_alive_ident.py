...
def TestSimInsertCard(connection):
    ...
    IsCardExist = False
    IsCardExist = HSUtil.IsCardExistInHW_List(2, 2, 'normal')
    
    if IsCardExist == True:
        print ">>> Sim Remove Card"
        HSUtil.SimDeadCard(2, 2)        
        sleep(300)
        ...
        IsCardExist = HSUtil.IsCardExistInHW_List(2, 2, 'major_error')
        if IsCardExist == True:
            print "Card exist"
        else:
