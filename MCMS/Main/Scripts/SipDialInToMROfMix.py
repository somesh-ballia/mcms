...
def AddMr(connection, mr_num, mrCreateFile, timeout):
    ...
    for x in range(mr_num):
        ...
        print "Adding New Mix Meeting Room : " + mrName + "  ..."
        connection.Send()
...
#------------------------------------------------------------------------------
def TestUndefinedDailIn(connection, num_of_parties, num_retries, conf_name = "mr1"):
    ### Add parties to EP Sim and connect them
    for x in range(num_of_parties):
        partyname = "Party"+str(x+1)
        if x < num_of_parties/2:
            print "Add AVC party..."
            connection.SimulationAddSipParty(partyname, conf_name)
        else:
            print "Add SVC party..."
            connection.SimulationAddSipParty(partyname, conf_name, "FULL CAPSET SVC")
    ...
    ConfId = connection.WaitUntillEQorMRAwakes(conf_name, num_of_parties,num_retries)
