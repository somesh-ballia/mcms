...
#*export MPL_SIM_FILE="VersionCfg/MPL_SIM_1.XML"
...
def TestSimInsertCard(connection):
    HSUtil = HotSwapUtils()
    test_if_Alert_exist(connection,"Resource","INSUFFICIENT_RESOURCES",0)
    IsCardExist = False
    IsCardExist = HSUtil.IsCardExistInHW_List(1, 2, 'normal')
    if IsCardExist == True:
    ...
    if IsCardExist == True:
        ...
        HSUtil.SimRemoveCard(1, 2)        
        sleep(2)
        
        test_if_Alert_exist(connection,"Resource","INSUFFICIENT_RESOURCES",1) 
        
        IsCardRemoved = HSUtil.IsCardRemovedFromHW_List(1, 2) 
        ...
        if IsCardExist == True:
            ...
            print "Test Failed!!! HotSwap not done, the card is  not there"
            exit (1)
