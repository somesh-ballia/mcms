...
for index in range(len(fault_elm_list)):
    fault_desc = fault_elm_list[index].getElementsByTagName("DESCRIPTION")[0].firstChild.data
    print fault_desc
    if fault_desc == "CARD_MANAGER_RECONNECT":
        reConnetion = reConnetion+1
            
if reConnetion == 0:
    print "Test failed,  no CARD_MANAGER_RECONNECT found in Faults after reconnect"
  #  sys.exit(1)

if reConnetion > 1:
    print "Test failed, More than 1 CARD_MANAGER_RECONNECT found in Faults after resconnect"
    sys.exit(1) 
    
if reConnetion == 1:
    print "Test passed, 1 CARD_MANAGER_RECONNECT found in Faults after reconnect"
