...
#### Name   	: Truncate SIP domain from site name #7
...
#### 2. P-Preferred-Identity contains sitename="TestSiteName" with "@.x.x.x.x".
#### 3. verify site name is TestSiteName.
#### Author		: Arbel M.
#### Reviewer   : Arbel M.
#### Date   	: 15/09/14
class TruncateSipDomainTest_7(SippTest):
	
	def GetSippXmlName(self):
		return "truncateSipDomain_7"
	#--------------------------------
	...
	def CheckTestResultFunc(self, sippTestUtils, LOG, strErrMsg):
		...
		if siteNameExists == -1:
			strErrMsg.append("Test FAILED - party visual name should be = TestSiteName")
			isTestPassed = 0;
			...
			if shtrudelExists != -1:
				strErrMsg.append("Test FAILED - party visual name should not contain the domain name \"@ \" ")
				isTestPassed = 0;
