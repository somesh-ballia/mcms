...
def CreateGW(connection,num_retries):
    ...
    print "Disconnecting in2  DialIn party"
    connection.SimulationDisconnectH323Party(partyName2)
    
    sleep(2)
    #Verify that GW conf has terminated
    print "Verify that GW_(01) conf has terminated:"
    connection.WaitConfEnd(gwId2,num_retries)
    
    #delete GW_00 conf
    print "delete GW_(00) conf"
    connection.DeleteConf(gwId1)
    sleep(2)
