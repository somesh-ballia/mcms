...
SAFE_UPGRADE_LOG=$MCU_HOME_DIR/tmp/startup_logs/softSafeUpgrade.log	
...
echo "safe_upgrade_log" $7 >> $SAFE_UPGRADE_LOG
...
#*********************************************************************************************************
cut_version=`echo $version_to_check | awk -F '.' '{print $1"."$2"."$3"."$4}'`
echo "cut_version" $cut_version >> $SAFE_UPGRADE_LOG
if [ "$cut_version" != "$version_to_check" ];
then
echo " version to check x1.x2.x3.x4---------------> "$cut_version  >> $SAFE_UPGRADE_LOG
#***********************************************************
#
#  check the black list in all products and all cards
#
