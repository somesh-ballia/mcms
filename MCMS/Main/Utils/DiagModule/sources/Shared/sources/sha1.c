...
static void sha1_process( sha1_context *ctx, char data[64] )
{
...
static const char sha1_padding[64] =
{
...
void sha1_finish( sha1_context *ctx, char *output )
{
    ...
    sha1_update( ctx, (char *) sha1_padding, padn );
...
static const char sha1_test_sum[3][20] =
{
...
int sha1_self_test( int verbose )
{
    ...
    for( i = 0; i < 3; i++ )
    {
        ...
        if( i < 2 )
            sha1_update( &ctx, (char *) sha1_test_str[i],
...
void base64 (const char *bin, size_t len, char *asciOut)
{
    ...
    while (j < len - 2)
    {
        asciOut[i++] = base64_table[bin[j] >> 2];
        asciOut[i++] = base64_table[((bin[j] & 3) << 4) | (bin[j + 1] >> 4)];
        asciOut[i++] = base64_table[((bin[j + 1] & 15) << 2) | (bin[j + 2] >> 6)];
        asciOut[i++] = base64_table[bin[j + 2] & 63];
        j += 3;
    }
    ...
    switch (len - j)
    {
        case 1:
            asciOut[i++] = base64_table[bin[j] >> 2];
            asciOut[i++] = base64_table[(bin[j] & 3) << 4];
            asciOut[i++] = BASE64_END;
            asciOut[i++] = BASE64_END;
            break;
        case 2:
            asciOut[i++] = base64_table[bin[j] >> 2];
            asciOut[i++] = base64_table[((bin[j] & 3) << 4) | (bin[j + 1] >> 4)];
            asciOut[i++] = base64_table[(bin[j + 1] & 15) << 2];
            asciOut[i++] = BASE64_END;
            break;
        case 0:
            break;
